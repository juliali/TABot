...
namespace Bot.Common.Utils
{
    public class DatetimeUtils
    {
        ...
        {           
            if (currentDatearea.StartsWith("XXXX-WXX-"))
            {
                ...
                else
                {
                    ...
                    {
                        /*string ruleW = "^[0-9]{4}-W[0-9]{2}$";
                        matchedValues = FileUtils.Match(rule, previousDateare);
                        if (matchedValues.Count > 0)
                        {
        ...
        public static string GetTodayWeekDayString(DateTime today)
        {
            switch(today.DayOfWeek)
            { 
        ...
        public static string GetOutofScopeAnswer(BotType type)
        {
            switch (type)
            {
                case BotType.WB:
                ...
                case BotType.PSA:
                    return "你好，我叫北宝，是个会说话的小机器人。我可以告诉你咱们班的课程和兴趣小组安排，还可以帮您记着今天老师布置的任务。有什么问题就请问吧[微笑]";
            }
        ...
        public static string GetCourseName(LUInfo luInfo)
        {
            List<string> coursenames = GetEntity(luInfo, "CourseItem");
